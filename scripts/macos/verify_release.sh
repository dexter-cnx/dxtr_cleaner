...
printf 'zip=%s\nverified_app=%s\nsha256=%s\nquarantine_required=%s\n' \
  "${ZIP_PATH}" "${APP_NAME}.app (extracted from ZIP)" "${SHA256}" "${QUARANTINE_REQUIRED}" \
  >"${STAGING_DIR}/summary.txt"

# Publish only a complete successful evidence bundle. Failed runs leave no stale success directory.
